...
const ADJUSTMENT_TYPES: { value: 'damaged' | 'returned'; label: string; description: string; color: string; dot: string }[] = [
  {
    value: 'damaged',
    label: 'Damaged',
      ...
      <div className="mx-auto max-w-2xl space-y-5">
        ...
        <div className="rounded-[28px] border border-white/80 bg-white/88 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500">Mark an item as damaged or returned by barcode</p>
          </div>
          ...
          <form onSubmit={e => { e.preventDefault(); handleSubmit(); }} className="space-y-5">
                  ...
                  <button
                    ...
                    className={`rounded-2xl border-2 px-4 py-4 text-left transition-all ${
                      form.status === type.value
                        ...
                        : 'border-slate-200 bg-slate-50/70 text-slate-600 hover:border-slate-300 hover:bg-slate-100/70'
                    }`}
                  >
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`h-2 w-2 rounded-full ${form.status === type.value ? type.dot : 'bg-slate-300'}`} />
            ...
            {/* Preview */}
            {form.barcode && (
              <div className={`rounded-2xl border px-5 py-4 ${selected.color}`}>
