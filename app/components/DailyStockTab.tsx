      ...
      <div className="mx-auto max-w-2xl space-y-5">
        ...
        <div className="rounded-[28px] border border-white/80 bg-white/88 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500">Select a product and date to view opening and closing stock</p>
          </div>
            ...
            <div>
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm outline-none transition hover:border-slate-300 focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100 flex items-center justify-between gap-3"
                >
                  {selected ? (
                    <span className="flex items-center gap-3">
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-xs font-bold text-sky-700">
                    ...
                    <div className="max-h-56 overflow-y-auto">
                      {filtered.length === 0 ? (
                        ...
                        <button
                          ...
                          className={`w-full text-left flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-sky-50/60 ${p.id === Number(productId) ? 'bg-sky-50' : ''}`}
                        >
        ...
        {stock && (
          <div className="rounded-[28px] border border-white/80 bg-white/88 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: 'Opening Stock', value: stock.opening, color: 'from-sky-500 to-blue-600', text: 'text-sky-700' },
                { label: 'Closing Stock', value: stock.closing, color: 'from-emerald-500 to-sky-500', text: 'text-emerald-700' },
                {
                  label: 'Movement',
                  value: movement >= 0 ? `+${movement}` : `${movement}`,
                  color: movement < 0 ? 'from-rose-500 to-orange-400' : 'from-emerald-500 to-teal-500',
                  text: movement < 0 ? 'text-rose-700' : 'text-emerald-700',
                },
              ].map(({ label, value, color, text }) => (
                <div key={label} className="rounded-2xl border border-slate-200 bg-slate-50/70 px-5 py-5">
