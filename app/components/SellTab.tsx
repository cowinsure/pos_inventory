      ...
      <div className="mx-auto max-w-4xl space-y-5">
        ...
        <div className="rounded-[28px] border border-white/80 bg-white/88 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
          ...
          <div className="mb-5">
            ...
            <p className="text-sm text-slate-500">Scan or type a barcode to add items to this sale</p>
          </div>
          ...
          <form onSubmit={e => { e.preventDefault(); handleScanBarcode(); }} className="flex gap-3">
            <div className="flex-1">
              <input
                ...
                disabled={searching}
                autoFocus
                className="w-full rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100 disabled:opacity-50"
              />
            </div>
            <button
              type="submit"
              disabled={!barcodeInput.trim() || searching}
              className="flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400"
            >
              {searching ? (
                <>
        ...
        {/* Customer card */}
        <div className="relative z-10 rounded-[28px] border border-white/80 bg-white/88 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
          <div className="mb-1.5 h-1 w-10 rounded-full bg-linear-to-r from-orange-400 to-sky-500" />
          <div className="mb-5">
            <div className="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600 mb-2">
              Customer
            </div>
            <p className="text-sm text-slate-500">Select an existing customer or create a new one</p>
          </div>
          ...
          <div className="relative mb-3" ref={dropdownRef}>
            <button
              ...
              className="w-full rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm text-slate-900 outline-none transition hover:border-slate-300 focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100 flex items-center justify-between"
            >
            ...
            {customerDropdownOpen && (
              <div className="absolute z-20 w-full mt-2 rounded-2xl border border-slate-200 bg-white shadow-[0_20px_50px_-24px_rgba(15,23,42,0.35)] overflow-hidden">
                ...
                <div className="max-h-52 overflow-y-auto">
                  {filteredCustomers.length === 0 ? (
                    ...
                    <button
                      ...
                      className="w-full text-left flex items-center gap-3 px-4 py-2.5 hover:bg-sky-50 transition-colors"
                    >
          ...
          <button
            ...
            className="w-full rounded-2xl border border-slate-200 bg-white/70 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 hover:border-slate-300"
          >
            {showCustomerForm ? 'Cancel' : '+ New Customer'}
          </button>

          {showCustomerForm && (
            <div className="mt-4 rounded-2xl border border-white/80 bg-slate-50/70 p-5 space-y-3">
              <div className="mb-1 h-0.5 w-8 rounded-full bg-linear-to-r from-sky-500 to-orange-400" />
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 mb-3">New customer details</p>
              {[
                ...
                <div key={field} className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100"
                  />
                </div>
              ))}
              <button
                ...
                className="w-full rounded-2xl bg-slate-950 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800"
              >
                Create Customer
              </button>
            </div>
        ...
        {totalItems > 0 ? (
          <div className="rounded-[28px] border border-white/80 bg-white/88 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
            <div className="mb-1.5 h-1 w-10 rounded-full bg-linear-to-r from-sky-500 to-orange-400" />
            <div className="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600 mb-5">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50">
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500 border-b border-slate-200">Barcode</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500 border-b border-slate-200">Product</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500 border-b border-slate-200">Price</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500 border-b border-slate-200">Discount</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.08em] text-slate-500 border-b border-slate-200">Notes</th>
                    <th className="px-4 py-3 border-b border-slate-200" />
                  </tr>
                </thead>
                <tbody>
                  {scannedItems.map((scanned, idx) => (
                    <tr key={scanned.item.id} className={`transition-colors hover:bg-sky-50/40 ${idx !== scannedItems.length - 1 ? 'border-b border-slate-100' : ''}`}>
                      <td className="px-4 py-3 font-mono text-xs text-slate-500">{scanned.item.barcode}</td>
                      <td className="px-4 py-3 text-sm font-semibold text-slate-900">{scanned.item.product?.name}</td>
                      <td className="px-4 py-3 text-sm text-slate-700">${Number(scanned.item.product?.basePrice || 0).toFixed(2)}</td>
                      <td className="px-4 py-3">
                        <input
                          ...
                          placeholder="0.00"
                          className="w-24 rounded-xl border border-slate-200 bg-slate-50/70 px-2.5 py-1.5 text-xs text-slate-900 outline-none focus:border-sky-400 focus:bg-white focus:ring-2 focus:ring-sky-100"
                        />
                      </td>
                      <td className="px-4 py-3">
                        <input
                          ...
                          placeholder="Notes..."
                          className="w-full rounded-xl border border-slate-200 bg-slate-50/70 px-2.5 py-1.5 text-xs text-slate-900 outline-none focus:border-sky-400 focus:bg-white focus:ring-2 focus:ring-sky-100"
                        />
                      </td>
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => removeItem(scanned.item.barcode)}
                          className="p-1.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors"
                          title="Remove"
                        >
            ...
            {/* Totals */}
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/70 p-5 space-y-2">
              <div className="flex items-center justify-between text-sm text-slate-600">
                <span>Subtotal ({totalItems} item{totalItems !== 1 ? 's' : ''})</span>
                <span className="font-medium text-slate-900">${cartTotals.totalPrice.toFixed(2)}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-600">Discount</span>
                <span className="font-medium text-rose-600">−${cartTotals.totalDiscount.toFixed(2)}</span>
              </div>
              <div className="flex items-center justify-between border-t border-slate-200 pt-3 mt-1">
                <span className="text-base font-semibold text-slate-900">Total</span>
                <span className="text-lg font-semibold text-slate-950">${cartTotals.subtotal.toFixed(2)}</span>
              </div>
            </div>
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-3.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400"
            >
              {loading ? (
                <>
                  <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24">
          ...
          <div className="rounded-[28px] border border-white/80 bg-white/88 p-12 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur text-center">
            ...
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Cart is empty</p>
            <p className="mt-2 text-sm text-slate-400">Scan a barcode above to start adding items</p>
          </div>
        )}
      </div>
    </div>
