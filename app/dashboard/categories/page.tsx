    ...
    <div className="min-h-[calc(100vh-8rem)] bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.10),transparent_35%),radial-gradient(circle_at_bottom_left,rgba(249,115,22,0.08),transparent_30%),linear-gradient(160deg,#f8fafc_0%,#eef6ff_50%,#fff7ed_100%)] p-6">
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        {rootCategories.length === 0 ? (
          <div className="rounded-[28px] border border-white/80 bg-white/88 p-12 text-center shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {rootCategories.map((cat) => (
              <div key={cat.id} className="rounded-[28px] border border-white/80 bg-white/88 p-6 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur">
                <div className="mb-1.5 h-1 w-10 rounded-full bg-linear-to-r from-sky-500 to-orange-400" />
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-sm font-bold text-sky-700">
                  ...
                  <button
                    ...
                    className="shrink-0 flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white/70 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-white hover:border-slate-300"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                    </svg>
                ...
                {cat.attributes?.length > 0 ? (
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {cat.attributes.map((attr) => (
                      <span key={attr.id} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
                        {attr.name}
                        {attr.options.length > 0 && (
                          <span className="text-slate-400">({attr.options.join(', ')})</span>
                        )}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${cat.active !== false ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-sm p-4" onClick={() => setShowModal(false)}>
          <div className="w-full max-w-md rounded-[28px] border border-white/80 bg-white/95 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.40)] backdrop-blur" onClick={e => e.stopPropagation()}>
            <div className="mb-1.5 h-1 w-10 rounded-full bg-linear-to-r from-sky-500 to-orange-400" />
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500">Fill in the details to create a category</p>
            </div>
                  ...
                  >
                    {selectedParent ? (
                      <span className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-slate-900">{selectedParent.name}</span>
                      </span>
                    ) : (
                    ...
                    <div className="absolute z-20 w-full mt-2 rounded-2xl border border-slate-200 bg-white shadow-[0_20px_50px_-20px_rgba(15,23,42,0.30)] overflow-hidden">
                      ...
                      <div className="max-h-52 overflow-y-auto">
                        ...
                        <button
                          ...
                          className={`w-full text-left flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-sky-50/60 ${!formData.parentId ? 'bg-sky-50' : ''}`}
                        >
                          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-xs font-bold text-slate-500">
                            —
                          </span>
                          <span className="text-sm text-slate-500">None — top-level</span>
                          {!formData.parentId && (
                        ...
                        {filteredParents.length === 0 && parentQuery ? (
                          ...
                          <button
                            ...
                            className={`w-full text-left flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-sky-50/60 ${formData.parentId === c.id.toString() ? 'bg-sky-50' : ''}`}
                          >
          ...
          <div className="w-full max-w-md rounded-[28px] border border-white/80 bg-white/95 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.40)] backdrop-blur" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500">
                Add attribute to <span className="font-semibold text-slate-700">{selectedCategory?.name}</span>
              </p>
            </div>

            {attrError && (
              <div className="mb-4 rounded-2xl bg-rose-50 border border-rose-100 px-4 py-3 text-sm text-rose-700">{attrError}</div>
            )}
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className="relative">
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`h-5 w-5 rounded-md border-2 flex items-center justify-center transition ${attrData.required ? 'bg-slate-950 border-slate-950' : 'border-slate-300 bg-white'}`}>
                      {attrData.required && (
