  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.10),transparent_35%),radial-gradient(circle_at_bottom_left,rgba(249,115,22,0.08),transparent_30%),linear-gradient(160deg,#f8fafc_0%,#eef6ff_50%,#fff7ed_100%)] dark:bg-none dark:bg-slate-950 p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {!loading && (
              <div className="inline-flex items-center gap-1.5 rounded-full bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-700/50 px-3 py-1 text-xs font-semibold text-orange-700 dark:text-orange-300">
          ...
          <button
            onClick={openCreate}
            className="flex items-center gap-2 rounded-2xl bg-slate-950 dark:bg-white px-5 py-2.5 text-sm font-semibold text-white dark:text-slate-900 transition hover:bg-slate-800 dark:hover:bg-slate-100"
          >
        ...
        {loading ? <Spinner /> : suppliers.length === 0 ? (
          <div className="rounded-[28px] border border-white/80 dark:border-slate-700/70 bg-white/88 dark:bg-slate-800/90 p-14 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.30)] backdrop-blur text-center">
            ...
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">No suppliers yet</p>
            <p className="mt-2 text-sm text-slate-400 dark:text-slate-500">Add your first supplier to start receiving inventory</p>
            <button
              onClick={openCreate}
              className="mt-5 rounded-2xl bg-slate-950 dark:bg-white px-6 py-2.5 text-sm font-semibold text-white dark:text-slate-900 transition hover:bg-slate-800 dark:hover:bg-slate-100"
            >
              Add Supplier
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {suppliers.map(supplier => (
              <div
                key={supplier.id}
                className="group rounded-[24px] border border-white/80 dark:border-slate-700/70 bg-white/88 dark:bg-slate-800/90 p-6 shadow-[0_20px_60px_-28px_rgba(15,23,42,0.22)] backdrop-blur transition hover:shadow-[0_28px_70px_-28px_rgba(15,23,42,0.30)]"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-100 dark:bg-orange-900/40 text-lg font-bold text-orange-700 dark:text-orange-300">
                      {supplier.name[0].toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <div className="mb-1 h-0.5 w-6 rounded-full bg-linear-to-r from-orange-400 to-sky-500" />
                      <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 truncate leading-snug">{supplier.name}</h3>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => openEdit(supplier)}
                      className="p-1.5 text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-xl transition-colors"
                      title="Edit"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="space-y-2.5 text-sm">
                  ...
                  {supplier.description && (
                    <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2 pt-2.5 border-t border-slate-100 dark:border-slate-700/50">
                      {supplier.description}
                    </p>
                  )}
                  {supplier.createdAt && (
                    <p className="text-xs text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-700/50">
      ...
      {showModal && (
        ...
        >
          <div
            className="w-full max-w-md rounded-[28px] border border-white/80 dark:border-slate-700/70 bg-white/95 dark:bg-slate-800/95 p-7 shadow-[0_40px_100px_-32px_rgba(15,23,42,0.45)] backdrop-blur"
          ...
          >
            ...
            <form onSubmit={e => { e.preventDefault(); handleSubmit(); }} className="space-y-4">
              {[
                ...
                <div key={field}>
                  <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">{label}{required && ' *'}</label>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  type="button"
                  onClick={closeModal}
                  className="rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 px-5 py-3 text-sm font-semibold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-700"
                >
                  Cancel
                </button>
              </div>
